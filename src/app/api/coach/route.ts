...
export async function POST(request: NextRequest) {
  try {
        ...
        input: [
          {
            ...
            content: [
              {
                type: "input_text",
                text: `You are CYG Coach inside a fitness tracking app. Answer from the user's supplied logged data. Be concise, practical, and explicit about uncertainty. Do not diagnose disease or present estimates as medical facts. Never silently change targets; frame adjustments as options the user can choose.\n\nQUESTION:\n${body.question}\n\nLOGGED CONTEXT:\n${JSON.stringify(body.context).slice(0, 60000)}`,
              },
            ],
          },
        ],
    ...
    console.error("CYG coach error:", error);
