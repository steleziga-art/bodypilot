...
export default function CoachPage({ goals, profile, nutritionHistory, weightEntries, trainingHistory, displaySettings, onOpenPlan }: { goals: Goals; profile: BodyProfile; nutritionHistory: NutritionDay[]; weightEntries: WeightEntry[]; trainingHistory: Workout[]; displaySettings: MucipesDisplaySettings; onOpenPlan: () => void }) {
  ...
  return <div className="space-y-5">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-blue-600">CYG Coach</p><h1 className="mt-1 text-4xl font-black tracking-tight">Your data, interpreted.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Coach prioritizes the signal that matters most from your logged training, nutrition and progress. You decide whether to apply any change.</p></div><button onClick={onOpenPlan} className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black">Open guided plan</button></header>

    <section className="rounded-[2rem] border border-blue-200 bg-blue-50 p-5 sm:p-6"><p className="text-xs font-black uppercase tracking-widest text-blue-700">Coach priority</p><h2 className="mt-2 text-2xl font-black text-slate-950">{priority.title}</h2><p className="mt-2 text-lg font-bold text-slate-800">{priority.detail}</p><p className="mt-2 text-sm leading-6 text-slate-600">{priority.action}</p></section>
    ...
    {insights.length > 1 && <section className="grid gap-3 md:grid-cols-3">{insights.slice(1).map((insight) => <article key={insight.title} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-widest text-blue-600">{insight.title}</p><p className="mt-3 font-black text-slate-950">{insight.detail}</p><p className="mt-2 text-sm leading-6 text-slate-500">{insight.action}</p></article>)}</section>}

    <section className="rounded-[2rem] border border-blue-200 bg-white p-5 text-slate-950 shadow-sm sm:p-6"><p className="text-xs font-black uppercase tracking-widest text-blue-600">Ask about your data</p><h2 className="mt-2 text-2xl font-black">What do you want to understand?</h2><div className="mt-3 flex flex-wrap gap-2">{["How was my week?","Am I close to my calorie target?","Is my training frequency on track?"].map((prompt)=><button key={prompt} type="button" disabled={loading} onClick={()=>void askCoach(prompt)} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 hover:border-blue-300 hover:bg-blue-50">{prompt}</button>)}</div><div className="mt-4 flex flex-col gap-2 sm:flex-row"><input value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void askCoach(); }} placeholder="Ask CYG Coach…" className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-base text-slate-950 outline-none focus:border-blue-400"/><button onClick={() => void askCoach()} disabled={loading || !question.trim()} className="rounded-2xl bg-blue-500 px-5 py-3 font-black text-white disabled:opacity-50">{loading ? "Thinking…" : "Ask"}</button></div>{answer && <div className="mt-4 whitespace-pre-wrap rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6 text-slate-700">{answer}</div>}</section>
  </div>;
}

function SummaryStat({ label, value }: { label: string; value: string | number }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-xs font-black uppercase tracking-wider text-slate-600">{label}</p><p className="mt-2 text-xl font-black text-slate-900">{value}</p></div>;
}
