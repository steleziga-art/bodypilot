      ...
      {socialTab === "friends" && <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 space-y-2">
          {accepted.map((row) => {
            ...
            return <button key={row.id} onClick={() => void openFriend(friendId)} className="flex w-full items-center gap-3 rounded-2xl bg-slate-50 p-3 text-left"><div className="grid h-10 w-10 place-items-center rounded-xl bg-white font-black text-emerald-700">{(person?.display_name || "F")[0].toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate font-black">{person?.display_name || "CYG user"}</p><p className="text-xs text-slate-500">@{person?.username || "user"}</p></div><span className="text-slate-400">›</span></button>;
          })}
