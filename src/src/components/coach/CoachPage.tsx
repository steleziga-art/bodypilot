...
function buildInsights({ goals, profile, nutritionHistory, weightEntries, trainingHistory, display }: { goals: Goals; profile: BodyProfile; nutritionHistory: NutritionDay[]; weightEntries: WeightEntry[]; trainingHistory: Workout[]; display: MucipesDisplaySettings }) {
  ...
  if (!insights.length) insights.push({ priority: 10, title: "Build your baseline", detail: "Log a few workouts, nutrition days and weigh-ins so Mucipes can compare trends.", action: "Consistent data matters more than adding more metrics." });
...
function localFallback(question: string, insights: Insight[]) {
  ...
  return `${relevant.detail}\n\n${relevant.action}\n\nThis is based on the data currently logged in Mucipes. More recent sessions, nutrition days and weigh-ins make the trend more representative.`;
}
