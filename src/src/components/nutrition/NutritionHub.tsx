...
type Tab = "today" | "insights" | "library";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex rounded-2xl bg-slate-100 p-1">
        {(["today", "insights", "library"] as const).map((name) => (
          <button
            key={name}
            onClick={() => setTab(name)}
          ...
          <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] bg-[#f7f9f8] p-4 shadow-2xl sm:p-6">
            ...
            {addMode === "menu" && (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <AddChoice title="What can I eat?" detail="Tell Mucipes what you have; it calculates portions." onClick={() => setAddMode("optimizer")} />
            ...
            {addMode === "scan" && (planTier === "premium" ? <MealScan meals={meals} defaultMealId={targetMealId} displaySettings={displaySettings} onAddFoods={(items) => { addFoods(items); setDiaryMessage("Scanned meal added."); }} /> : <PremiumPaywall title="AI Meal Scan is Premium" detail="Photo-based meal estimation is available on Mucipes Premium. You can still search foods, use barcode lookup and quick add on Free." />)}
