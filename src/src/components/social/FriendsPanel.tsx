...
export default function FriendsPanel() {
  ...
  async function refresh() {
    ...
    const defaultName = user.user_metadata?.display_name || user.email?.split("@")[0] || "Mucipes user";
  ...
  if (!me) return <div className="rounded-3xl border border-slate-200 bg-white p-6"><h2 className="text-xl font-black">Sign in first</h2><p className="mt-2 text-sm text-slate-500">Friends use your Mucipes cloud account so requests and privacy rules work across devices.</p></div>;
  if (setupMissing) return <div className="rounded-3xl border border-amber-200 bg-amber-50 p-6"><h2 className="text-xl font-black text-amber-950">Friends database setup is required once</h2><p className="mt-2 text-sm leading-6 text-amber-800">Run the included <b>MUCIPES_V1_SOCIAL.sql</b> migration in Supabase, then reopen this page. No existing workout or nutrition data is deleted.</p></div>;
      ...
      <div className="space-y-5">
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-4"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-emerald-100 text-xl font-black text-emerald-700">{(social?.display_name || "F")[0].toUpperCase()}</div><div><h2 className="text-2xl font-black">{social?.display_name || "Friend"}</h2><p className="text-sm text-slate-500">@{social?.username || "user"}</p></div></div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black uppercase tracking-widest text-slate-400">What they can see from you</p>
          ...
          <p className="mt-3 text-xs text-slate-400">Private categories stay hidden until you explicitly enable them for this friend.</p>
        </section>

        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-emerald-600">Shared with you</p><h3 className="mt-1 text-xl font-black">Friend activity</h3></div><button onClick={() => void openFriend(selectedFriend)} className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">Refresh</button></div>
  ...
  return (
    <div className="space-y-5">
      ...
      {socialTab === "friends" && <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-xs font-black uppercase tracking-widest text-emerald-600">Your social profile</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
          <label><span className="text-xs font-black text-slate-500">Username</span><div className="mt-1 flex rounded-xl border border-slate-200"><span className="self-center pl-3 text-slate-400">@</span><input value={username} onChange={(event) => setUsername(event.target.value)} className="min-w-0 flex-1 rounded-xl px-2 py-3 outline-none" /></div></label>
          ...
          <button onClick={() => void saveProfile()} className="self-end rounded-xl bg-emerald-500 px-5 py-3 text-sm font-black text-white">Save</button>
        </div>
        {profile && <p className="mt-3 text-xs text-slate-400">Friends can find you as @{profile.username}. Your private fitness data is not public.</p>}
      </section>}
      ...
      {socialTab === "search" && <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 flex gap-2"><input value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void searchUsers(); }} placeholder="Search @username or name" className="min-w-0 flex-1 rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-emerald-500" /><button type="button" disabled={searching} onClick={() => void searchUsers()} className="rounded-xl bg-emerald-500 px-5 text-sm font-black text-white disabled:opacity-50">{searching ? "Searching…" : "Search"}</button></div>
        ...
        {results.length > 0 && <div className="mt-3 space-y-2">{results.map((item) => <div key={item.user_id} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3"><div className="min-w-0 flex-1"><p className="font-black">{item.display_name}</p><p className="text-xs text-slate-500">@{item.username}</p></div><button onClick={() => void requestFriend(item.user_id)} className="rounded-xl bg-white px-3 py-2 text-xs font-black text-emerald-700">Add friend</button></div>)}</div>}
      </section>}

      {socialTab === "requests" && <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-black">Requests</h2>{incoming.length === 0 && outgoing.length === 0 ? <p className="mt-3 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">No pending friend requests.</p> : null}{incoming.length > 0 && <div className="mt-3 space-y-2">{incoming.map((row) => { const person = profiles[row.requester_id]; return <div key={row.id} className="flex flex-wrap items-center gap-3 rounded-2xl bg-slate-50 p-3"><div className="min-w-0 flex-1"><p className="font-black">{person?.display_name || "Mucipes user"}</p><p className="text-xs text-slate-500">@{person?.username || "user"}</p></div><button type="button" onClick={() => void setRequest(row.id, "declined")} className="rounded-xl px-3 py-2 text-xs font-black text-rose-600">Decline</button><button type="button" onClick={() => void setRequest(row.id, "accepted")} className="rounded-xl bg-emerald-500 px-3 py-2 text-xs font-black text-white">Accept</button></div>; })}</div>}{outgoing.length > 0 && <div className="mt-5"><p className="text-xs font-black uppercase tracking-widest text-slate-400">Sent</p><div className="mt-2 space-y-2">{outgoing.map(row => { const person=profiles[row.addressee_id]; return <div key={row.id} className="rounded-2xl bg-slate-50 p-3"><p className="font-black">{person?.display_name || "Mucipes user"}</p><p className="text-xs text-slate-500">@{person?.username || "user"} · Pending</p></div>; })}</div></div>}</section>}
      ...
      {socialTab === "friends" && <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 space-y-2">
          {accepted.map((row) => {
            ...
            return <button key={row.id} onClick={() => void openFriend(friendId)} className="flex w-full items-center gap-3 rounded-2xl bg-slate-50 p-3 text-left"><div className="grid h-10 w-10 place-items-center rounded-xl bg-white font-black text-emerald-700">{(person?.display_name || "F")[0].toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate font-black">{person?.display_name || "Mucipes user"}</p><p className="text-xs text-slate-500">@{person?.username || "user"}</p></div><span className="text-slate-400">›</span></button>;
          })}
        ...
        {outgoing.length > 0 && <p className="mt-3 text-xs text-slate-400">{outgoing.length} outgoing request{outgoing.length === 1 ? "" : "s"} pending.</p>}
      </section>}
...
function FriendSnapshotView({ snapshot }: { snapshot: FriendSnapshot }) {
  ...
  return <div className="mt-4 grid gap-3 sm:grid-cols-2">
    {snapshot.training && <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-widest text-slate-400">Training</p><p className="mt-2 text-2xl font-black">{snapshot.training.weeklyWorkouts ?? 0} <span className="text-sm text-slate-400">this week</span></p>{snapshot.training.latestWorkout && <p className="mt-2 text-sm text-slate-500">Latest: {snapshot.training.latestWorkout.name} · {snapshot.training.latestWorkout.durationMinutes} min</p>}{snapshot.training.prs?.slice(0, 3).map((pr) => <p key={`${pr.exercise}-${pr.weight}`} className="mt-2 text-xs font-bold text-slate-600">PR · {pr.exercise}: {pr.weight} kg × {pr.reps}</p>)}</div>}
    {snapshot.progress && <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-widest text-slate-400">Progress</p><p className="mt-2 text-2xl font-black">{snapshot.progress.latestWeight ?? "—"} <span className="text-sm text-slate-400">kg</span></p><p className="mt-2 text-sm text-slate-500">Latest change: {snapshot.progress.weightChange === null || snapshot.progress.weightChange === undefined ? "—" : `${snapshot.progress.weightChange > 0 ? "+" : ""}${snapshot.progress.weightChange} kg`}</p></div>}
    {snapshot.nutrition && <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-widest text-slate-400">Nutrition</p><p className="mt-2 text-2xl font-black">{snapshot.nutrition.todayCalories ?? 0} <span className="text-sm text-slate-400">/ {snapshot.nutrition.calorieGoal ?? 0} kcal</span></p><p className="mt-2 text-sm text-slate-500">Protein {snapshot.nutrition.protein ?? 0} / {snapshot.nutrition.proteinGoal ?? 0} g</p></div>}
    {snapshot.measurements && <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-widest text-slate-400">Measurements</p>{snapshot.measurements.latest ? <p className="mt-2 text-sm font-bold">Waist {snapshot.measurements.latest.waist ?? "—"} · Chest {snapshot.measurements.latest.chest ?? "—"} · Arm {snapshot.measurements.latest.arm ?? "—"} cm</p> : <p className="mt-2 text-sm text-slate-500">No measurements shared.</p>}</div>}
  </div>;
}
